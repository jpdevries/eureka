...
const MediaDirectorySelector = (props) => {
  const optgroups = props.directory.map((source, index) => { // todo: rename props.directory to something that makes more sense (or combine it with the sources reducer?)
    const opts = source.directories.map((directory) => (
      <option value={`${source.id}__${directory.cd}`}>{directory.cd}</option>
    ));
    return (
      <optgroup label={source.name} data-source={source.id === undefined ? index : source.id}>
  ...
  return (
    <div className="eureka__media-directory-selector">
      <label htmlFor="eureka__media-browser_0__browsing">Browse Directory:</label>
      <form aciton="#">
