...
const ContextButtons = (props) => {
  ...
  return (
    <div className="eureka__button-bar eureka__context-buttons">
      ...
      <button title={`Rename ${item.filename}`} onClick={props.renameStart ? props.renameStart.bind(null, item) : undefined}>Rename<span className="visually-hidden"> {item.filename}</span></button>
